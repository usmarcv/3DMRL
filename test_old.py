...
from models.clip_adapter import NewCLIP, Adapter
...
def main(cli_args, extras):
    ...
    if config.train:
        ...
        image_alignmemt_adapter = image_alignment_adapter.to('cuda')
        ...
        if config.trainer == "trainer":
            ...
            if config.use_cia:
                ...
                pretrained_clip_adapter.load_state_dict(new_state_dict, strict=True) 
            else:
                pretrained_clip_adapter = NewCLIP(config.pretrained_clip.c_in,
                                                  ratio=config.pretrained_clip.ratio).cuda()
