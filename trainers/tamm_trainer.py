...
class TAMM_Trainer(object):
    ...
    def load_from_checkpoint(self, path):
        ...
        # 2. Load Main Model (with prefix added)
        self.model.load_state_dict(add_prefix(checkpoint['state_dict']))

        self.image_alignment_adapter.load_state_dict(add_prefix(checkpoint['image_alignment_adapter']))
        self.text_alignment_adapter.load_state_dict(add_prefix(checkpoint['text_alignment_adapter']))
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.train_loader):
            ...
            if not self.config.model.get("use_dense", False):
                #  pred_feat = self.model(data['xyz'].to(self.config.device), \
                #                         data['features'].to(self.config.device))
