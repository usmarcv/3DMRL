...
class TAMM_Trainer_MRL(object):
    ...
    def contrast_mrl_loss(self, feat1, feat2, mlr_dim, logit_scale=1, mask=None):
        ...
        #nesting only feats from point cloud
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.train_loader):            
            ...
            if self.config.training.loss_type == "two_branch" and self.config.mrl.get("use_mrl", True):            
                    for i in range(self.config.dataset.num_imgs):
                        ...
                        # single_img_feat = single_img_feat.to(self.config.dzevice)
                        ...
                        self.contrast_mrl_loss(pc_image_feat, single_img_feat,
                    ...
                    for i in range(self.config.dataset.num_texts):
                        ...
                        txt_loss_dims, txt_acc_dims = \
                        self.contrast_mrl_loss(pc_text_feat[idx], single_text_feat,
                                                self.config.mrl.dims,
        ...
        if self.rank == 0:
            logging.info('Train avg: image_contrast_acc: {0} text_contrast_acc: {1}' \
                         .format(np.mean(img_contras_acc_list) / len(self.config.mrl.dims), \
                                 np.mean(text_contras_acc_list) / len(self.config.mrl.dims) if len(text_contras_acc_list) > 0 else 0))
