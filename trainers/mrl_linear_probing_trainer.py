...
import wandb
# Removed: from numpy import * (To avoid overriding Python's native sum())
...
class Linear_Probing_Trainer(object):
    ...
    def load_from_checkpoint(self, path):
        ...
        self.linear_layer.load_state_dict(checkpoint['state_dict'])
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.train_loader, desc=f"Train Epoch {self.epoch}" if self.rank == 0 else None):
            ...
            # Extração de features não consome memória de gradiente
            with torch.no_grad():
                ...
                # Extrai o tensor se for tupla
                if isinstance(pred_feat, tuple):
                    pred_feat = pred_feat[0]

                # Projeta para o espaço MRL
            ...
            # ==========================================
            # Apenas a linear_layer recebe gradiente
            # ==========================================
        ...
        for dim_str in self.mrl_dims:
            ...
            if self.rank == 0:
                ...
                if overall_acc_val > self.best_overall_acc[dim_str]:
                    ...
                    self.save_model(f'best_{dataset_name}_overall_dim_{dim_str}')
                    
                if class_acc_val > self.best_class_acc[dim_str]:
                    self.best_class_acc[dim_str] = class_acc_val
                    self.save_model(f'best_{dataset_name}_class_dim_{dim_str}')
        ...
        if self.rank == 0:
            ...
            logging.info(f"Test Results Epoch {self.epoch}:{table_str}")
    ...
    def train(self):
        for epoch in range(self.epoch, self.config.training.max_epoch):
            ...
            # CORREÇÃO 2: Atualizar o sampler do DDP para garantir o shuffle correto entre as épocas
            if hasattr(self.train_loader, 'sampler') and hasattr(self.train_loader.sampler, 'set_epoch'):
                self.train_loader.sampler.set_epoch(epoch)

            # Roda o treino e teste da época atual
            self.train_one_epoch()
            
            if self.rank == 0:
                self.save_model('latest')
            if self.rank == 0 and self.epoch > 0 and self.epoch % self.config.training.save_freq == 0:
